...
## compression options
[[ -z $compression ]] && compression="-comp xz -no-exports -b 256M"
...
[[ "$openbox" ]] && systemctl enable slock@$user
...
if [[ $mkinitcpio_passwd_hash ]]; then
	...
	while true; do
		read -rsp 'Enter boot password > ' pass1
		read -rsp 'Confirm password > ' pass2
		[[ "$pass1" = "$pass2" ]] && break
		echo "Passwords don't match! Try again."
	done
	pass1=$(echo "$pass1" | sed -E 's/\\/\\\\/g;s/  -$//') # fix backslashes for sed and clean output
	sed -Ei "s/PASSWD_HASH/$pass1/" "$wdir/initcpio/hooks/passwd"
fi
...
mount -t overlay hooks -o "lowerdir=/etc/initcpio:$mdir/work/initcpio" /etc/initcpio
